...
define([
...
], function (requirejs, $, Jupyter, events, codecell, toc2) {
  "use strict";
  ...
  function addSaveAsWithToc() {
    if (parseFloat(Jupyter.version.substr(0, 3)) >= 5.1) {
      if ($("#download_html_toc").length === 0) {
        ...
        var downloadEntry = $(
          '<li id="download_html_toc"><a href="#">HTML with toc (.html)</a></li>'
        );
      ...
      /* Add a "save a" menu entry for pre 5.1 versions (needs python kernel) */
  ...
  function create_additional_css(cfg) {
    ...
    sheet.innerHTML =
      "#toc li > span:hover { background-color: " +
      cfg.colors.hover_highlight +
      " }\n" +
      ".toc-item-highlight-select  {background-color: " +
      cfg.colors.selected_highlight +
      "}\n" +
      ".toc-item-highlight-execute  {background-color: " +
      cfg.colors.running_highlight +
      "}\n" +
      ".toc-item-highlight-execute.toc-item-highlight-select   {background-color: " +
      cfg.colors.selected_highlight +
      "}";
    if (cfg.moveMenuLeft) {
      sheet.innerHTML +=
    ...
    sheet.innerHTML +=
      ...
      "}\n";
    sheet.innerHTML +=
      "#toc a, #navigate_menu a, .toc { color: " +
      cfg.colors.navigate_text +
      "}";
    sheet.innerHTML +=
      "#toc-wrapper .toc-item-num { color: " + cfg.colors.navigate_num + "}";
    sheet.innerHTML +=
      ".sidebar-wrapper { border-color: " + cfg.colors.sidebar_border + "}";
    sheet.innerHTML +=
      ".highlight_on_scroll { border-left: solid 4px " +
      cfg.colors.on_scroll +
      "}";
    ...
    CodeCell.prototype.get_callbacks = function () {
      ...
      callbacks.shell.reply = function (msg) {
        if (msg.msg_type === "execute_reply") {
          setTimeout(function () {
            $(".toc .toc-item-highlight-execute").removeClass(
  ...
  function rehighlight_running_cells() {
    $.each(
      $(".running"), // re-highlight running cells
      function (idx, elt) {
        highlight_toc_item(
          {
            type: "execute",
