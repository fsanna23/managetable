(requirejs.specified("base/js/namespace")
  ...
  var callback_toc_link_click = function (evt) {
    ...
    var currentSection = $("#toc  .highlight_on_scroll a").data(
      "tocModifiedId"
    );
    ...
    if (liveNotebook) {
      ...
      // like . in an id
      var cell = $(document.getElementById(trg_id))
        .closest(".cell")
        .data("cell");
  ...
  window.addEventListener("popstate", function (e) {
    if (e.state != null && e.state.back != null) {
      ...
      if (liveNotebook) {
        var cell = $(document.getElementById(back_id))
          .closest(".cell")
          .data("cell");
        Jupyter.notebook.select(Jupyter.notebook.find_cell_index(cell));
        highlight_toc_item("toc_link_click", {
  ...
  function highlight_toc_item(evt, data) {
    ...
    if (evt.type === "execute") {
      ...
      $(".toc .toc-item-highlight-select").removeClass(
  ...
  var create_navigate_menu = function (callback) {
    ...
    $("#Navigate")
      .addClass("dropdown")
      .append($("<a/>").attr("href", "#").attr("id", "Navigate_sub"));
    $("#Navigate_sub")
      .text("Navigate")
    ...
    if (IPython.notebook.metadata.toc["nav_menu"]) {
      ...
      events.on("before_save.Notebook", function () {
        try {
          IPython.notebook.metadata.toc.nav_menu["width"] =
            $("#Navigate_menu").css("width");
          IPython.notebook.metadata.toc.nav_menu["height"] =
            $("#Navigate_menu").css("height");
        } catch (e) {
          console.log(
            "[toc2] Error in metadata (navigation menu) - Proceeding",
            e
          );
        }
      });
  ...
  var saveTocPosition = function () {
    ...
    var new_values = toc_wrapper.hasClass("sidebar-wrapper")
      ? ["width"]
  ...
  var makeUnmakeMinimized = function (cfg, animate) {
    ...
    if (open) {
      ...
      new_css = cfg.sideBar
        ? {}
        : { height: toc_position.height, width: toc_position.width };
    } else {
      new_css = {
    ...
    wrap
      ...
      .find(".hide-btn")
      .attr("title", open ? "Hide ToC" : "Show ToC");
    return open;
  };
  ...
  var makeUnmakeSidebar = function (cfg) {
    ...
    var view_rect = (
      liveNotebook ? document.getElementById("site") : document.body
    ...
    wrap
      ...
      .toggleClass("ui-icon ui-icon-grip-dotted-vertical", make_sidebar);
    if (make_sidebar) {
    ...
    if (cfg.markTocItemOnScroll) {
      ...
      $(scrolling_elt).scroll(function () {
        ...
        var bottom_of_screen =
          $(window).scrollTop() + $(scrolling_elt).height() + headerHeight;
  ...
  var collapse_by_id = function (trg_id, show, trigger_event) {
    var anchors = $(".toc .toc-item > li > span > a").filter(function (
      idx,
      elt
    ) {
      return $(elt).attr("data-toc-modified-id") === trg_id;
  ...
  var callback_toc2_collapsible_headings = function (evt, data) {
    var trg_id = data.cell.element
      .find(":header")
      .filter(function (idx, elt) {
  ...
  var table_of_contents = function (cfg, st) {
    ...
    if (toc_wrapper.length === 0) {
      // toc window doesn't exist at all
    ...
    // or the ToC cell.
    all_headers = $(".text_cell_render").find(
      "[id]:header:not(:has(.tocSkip))"
    );
    ...
    all_headers.each(function (i, h) {
      ...
      ul.append($("<li>").append($("<span>").append(make_link(h, toc_mod_id))));
    });

    // update navigation menu
    if (cfg.navigate_menu) {
      var pop_nav = function () {
        //callback for create_nav_menu
      ...
      // If navigate_menu is false but the menu already exists, then remove it
    ...
    events[cfg.collapse_to_match_collapsible_headings ? "on" : "off"](
      "collapse.CollapsibleHeading uncollapse.CollapsibleHeading",
      callback_toc2_collapsible_headings
    );
  };
  ...
  var show_settings_dialog = function (cfg, st) {
    ...
    var build_setting_input = function (md_key, md_label, input_type) {
      ...
      var fg = $("<div>").append($("<label>").text(md_label).attr("for", id));
      var input = $("<input/>")
        .attr({
      ...
      if (input_type == "checkbox") {
        ...
        input.prop("checked", opts[md_key]).prependTo(fg.children("label"));
      } else {
        fg.addClass("form-group");
        input.addClass("form-control").val(opts[md_key]).appendTo(fg);
      }
      return fg;
    };
      ...
      .append([
        $("<div>").text(
          "These settings apply to this notebook only, and are stored in its metadata. " +
            liveNotebook
            ? "The defaults for new notebooks can be edited from the nbextensions configurator."
        ...
        build_setting_input(
          "sideBar",
          "Display as a sidebar (otherwise as a floating window)",
          "checkbox"
        ),
        build_setting_input(
          "toc_window_display",
        ...
        build_setting_input(
          "toc_section_display",
          "Expand window/sidebar at startup",
          "checkbox"
        ),
      ])
      .appendTo(dialog_content);
    $('<div class="modal-footer">')
      .append(
        '<button class="btn btn-default btn-sm btn-primary" data-dismiss="modal">Ok</button>'
      )
      .appendTo(dialog_content);
    ...
    try {
      ...
      var $backdrop = $('<div class="modal-backdrop fade">').appendTo(
        $(document.body)
      );
