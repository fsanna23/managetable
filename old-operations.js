const operations = [
  ...
  },
  {
    ...
    insert_code: function () {
      ...
      const code = `
${df} = ${df}.drop([${rowIndices}]).reset_index(drop=True)
${df}`;
      // insert_code_to_nb(code.trim());
      insert_code_and_markdown_to_nb(code, markdown);
    },
  },
  {
    id: 5,
    ...
    create_input: function () {
      ...
      createTableSelector("manager-select-df", null, "Select a table", null);
    },
    insert_code: function () {
      const df = $("#manager-select-df").val();
      ...
      const code = `
indices = [x for x in range(1, len(${df}) + 1)]
...
${df}`;
  ...
  },
  {
    ...
    insert_code: function () {
      const df = $("#manager-select-df").val();
      const columnsToRename = $.map($(".rename-col-table"), function (option) {
        return option.value;
      });
        ...
        getColumnsFromTable(dfs[0]).then((tableColumns) => {
          createSelect(
            ...
            function (event) {
              /* Whenever a column is selected, it needs to be removed from the other selects. */
              const newValue = event.target.value;
              const newValues = tableColumns.filter((el) => el !== newValue);
              if (
                $(
                  "#move-column-select-column-before, #move-column-select-column-after"
                ).length !== 0
          ...
          createSelect(
            ...
            function (event) {
              ...
              if (
                ...
                getColumnsFromTable().then((tableColumns) => {
                  const selectCol = $("#move-column-select-column").val();
                  tableColumns = tableColumns.filter((el) => el !== selectCol);
                  createSelect(
                    selectedOp === "Move before column"
                      ? "move-column-select-column-before"
                      : "move-column-select-column-after",
    ...
    create_input: function () {
      ...
      const generateInput = (df) => {
        getColumnsFromTable(df).then((tableColumns) => {
            ...
            with the correct amount of inputs, based on the value of this numerical input. */
              ...
              if (inputValues.length === 1) {
                ...
                inputValues.each(function (index) {
                  ...
                  const difference = Math.abs(newValuesEl.length - newNumber);
          ...
          createButton("expand-row-add-value", "Add new value", function () {
...
${df}`;
  ...
  },
  {
    ...
    updates: true,
    create_input: function () {
      createTableSelector("manager-select-df1", null, "Select the first table");
      createTableSelector(
        "manager-select-df2",
        null,
